    ...
    private UsernamePasswordAuthenticationToken getAuthentication(HttpServletRequest request) {
        ...
        if (token != null) {
            ...
            String user = JWT.require(Algorithm.HMAC512(SECRET))
                    ...
                    .getSubject();
            Claim authorityClaim = JWT.require(Algorithm.HMAC512(SECRET))
                    .build()
                    .verify(token.replace(TOKEN_PREFIX, ""))
            ...
            if (user != null) {
                if(authorityClaim != null){
                    ...
                    if(!authorities.isEmpty()){
                        UsernamePasswordAuthenticationToken auth = new UsernamePasswordAuthenticationToken(user,null,authorities.stream().map(SimpleGrantedAuthority::new).collect(Collectors.toList()));
